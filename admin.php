...
if ($id == "login" || substr($url, -1) == "p") {
  ...
  if ($API->connect($iphost, $userhost, decrypt($passwdhost))){
    ...
    if ($currency == in_array($currency, $cekindo['indo'])) {
      echo "<script>alert('Golden Wifi not connected!".$nl."Silakan periksa kembali IP, User, Password dan port API harus enable.".$nl."Jika menggunakan koneksi VPN, pastikan VPN tersebut terkoneksi.')</script>";
    }else{
      echo "<script>alert('Golden Wifi not connected!".$nl."Please check the IP, User, Password and port API must be enabled.')</script>";
    }
    if($c == "settings"){
